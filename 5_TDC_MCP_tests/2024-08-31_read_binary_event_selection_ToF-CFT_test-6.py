...
for sweep_number, sweep_dict in dict_data_sweep.items():
    ...
    if len(sweep_dict) != 10:
        ...
        # Rising edge events (STOP 1, 2, 3, 4, 5)
...
for sweep_number, sweep_dict in dict_data_sweep.items():
    ...
    for event_time, event_dict in sweep_dict.items():
        ...
        if event_dict['edge_w'] == 'RISING':
            if event_dict['channel_w'] == 'STOP_1':
                times[0] = event_dict['time']
                t_E_rising = event_dict['time']
            elif event_dict['channel_w'] == 'STOP_2':
                times[2] = event_dict['time']
        elif event_dict['edge_w'] == 'FALLING':
            if event_dict['channel_w'] == 'STOP_1':
                times[1] = event_dict['time']
                t_E_falling = event_dict['time']
    
    times[3] = t_E_rising + 0.2 * (t_E_falling - t_E_rising)
    times[4] = t_E_rising + 0.3 * (t_E_falling - t_E_rising)
    times[5] = t_E_rising + 0.4 * (t_E_falling - t_E_rising)
...
for measurement_number, measurement_dict in dict_data_timing.items():
    ...
    vector_3.append(measurement_dict['ToF'][2])

this_histo, edges = np.histogram(vector_1, bins=100, range=(41.93, 42.01))
ax1.step(edges[:-1], this_histo, where='post')
...
this_histo, edges = np.histogram(vector_2, bins=100, range=(42.0, 42.2))
ax2.step(edges[:-1], this_histo, where='post')
...
this_histo, edges = np.histogram(vector_3, bins=100, range=(42.05, 42.4))
ax3.step(edges[:-1], this_histo, where='post')
...
vector_3 = []
for measurement_number, measurement_dict in dict_data_timing.items():
    vector_1.append(measurement_dict['ToF'][3])
    vector_2.append(measurement_dict['ToF'][4])
    vector_3.append(measurement_dict['ToF'][5])

this_histo, edges = np.histogram(vector_1, bins=100, range=(41.93, 42.05))
...
this_histo, edges = np.histogram(vector_2, bins=100, range=(41.93, 42.05))
ax.step(edges[:-1], this_histo, where='post')
...
this_histo, edges = np.histogram(vector_3, bins=100, range=(41.93, 42.05))
ax.step(edges[:-1], this_histo, where='post')
